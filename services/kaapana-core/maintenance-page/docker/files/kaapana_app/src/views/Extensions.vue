      ...
      v-data-table.elevation-1(
        ...
        template(v-slot:item.installed="{ item }")
          ...
          v-btn(
            ...
            v-dialog(
              v-if="item.extension_params !== undefined",
              v-model="popUpDialog",
              :retain-focus="false",
              max-width="600px"
            )
...
export default Vue.extend({
    ...
    filteredLaunchedAppLinks(): any {
      if (this.launchedAppLinks !== null) {
        return this.launchedAppLinks.filter((i: any) => {
          ...
          if (
            this.extensionExperimental == "Stable" &&
            i.experimental === "yes"
          ) {
            devFilter = false;
          } else if (
  ...
  methods: {
    ...
    getFormInfo(item: any) {
      ...
      if (
        item["extension_params"] &&
        Object.keys(item["extension_params"]).length > 0
      ) {
    ...
    submitForm() {
      ...
      if (
        (this.$refs.popUpForm as Vue & { validate: () => boolean }).validate()
      ) {
        this.popUpDialog = false;
