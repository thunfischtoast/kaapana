...
from bdc_federated.BreastDensityClassifierFederatedOperator import BreastDensityClassifierFederatedOperator

log = LoggingMixin().log

# FL releated
remote_dag_id = "breast-density-classification"     # name of DAG which should be executed in a federated way
...
args = {
    'ui_visible': False,    # DAG cannot be selected in Meta-Dashboard
...
bdc_federated = BreastDensityClassifierFederatedOperator(dag=dag,
                                                        # dev_server='code-server',
                                                        )

put_to_minio = LocalMinioOperator(dag=dag,
                                 action='put',
                                 action_operators=[bdc_federated],
                                 zip_files=True,
...
bdc_federated >> put_to_minio >> clean
