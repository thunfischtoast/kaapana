...
class BreastDensityClassificationFederatedTraining(KaapanaFederatedTrainingBase):
    ...
    @timeit
    def update_data(self, federated_round, tmp_central_site_info):  # is called by KaapanaFederatedTrainingBase's central_steps() method ; method overwrites empty update_data() method of KaapanaFederatedTrainingBase
        ...
        if federated_round == -1:   # changed -2 to -1 due to having only 1 preprocessing round
            ...
            if self.remote_conf_data['federated_form']['federated_total_rounds'] == federated_round+1 and self.use_minio_mount is not None:
                src = current_federated_round_dir / self.remote_sites[0]['instance_name'] / 'breast_density_classifier' 
                dst = os.path.join('/', self.workflow_dir, 'breast_density_classifier')
                print(f'Last round! Copying final bdc-training files from {src} to {dst}')
    ...
    @timeit
    def on_wait_for_jobs_end(self, federated_round):    # called by KaapanaFederatedTrainingBase's method train_step()
...
if __name__ == "__main__":
    ...
    kaapana_ft = BreastDensityClassificationFederatedTraining(use_minio_mount='/minio') # instantiate FederatedTraining class
